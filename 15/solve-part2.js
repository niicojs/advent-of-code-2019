...
function floodfill() {
  let time = 0;
  const target = [...map.values()].filter((v) => ['o', ' '].includes(v)).length;
  const fill = new Set([key(...oxygen)]);
  while (fill.size < target) {
    const vals = [...fill].map((k) => k.split(',').map(Number));
    for (const [x, y] of vals) {
      getDirectNeighbors(x, y).forEach(([nx, ny]) => {
        if (map.get(key(nx, ny)) === ' ') {
          fill.add(key(nx, ny));
        }
      });
    }
